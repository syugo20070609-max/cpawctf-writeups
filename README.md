...
| 種類 | 名前 | 用途 |
|---|---|---|
| Linux | `file` | ファイルの種類を調べる |
| Linux | `chmod +x` | 実行権限を付与する |
| Linux | `ls` / `ls -l` | ファイル一覧を表示する |
| Linux | `mkdir -p` | ディレクトリを作成する |
| Linux | `mv` | ファイル・フォルダを移動/リネームする |
| Python | `sort(reverse=True)` | リストを降順ソートする |
| Python | `replace()` | 文字列の不要な文字を除去する |
...
| ネットワーク | パケット・pcapの基礎 |
| PPC | プログラムで解く問題。手作業で無理な量はコードで自動化する |
...
| PPC | 並べ替えろ! | Python | sort(reverse=True)で降順ソート、replaceで文字列整形 |
